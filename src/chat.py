    ...
    async def process_query(self, message: str, k: int = 5) -> Dict[str, Any]:
        ...
        while retries <= self.max_retries:
            try:
                ...
                retries += 1
                last_error = e
                logger.warning(f"Error in chat processing (attempt {retries}/{self.max_retries}): {str(e)}")
        ...
        logger.error(f"Failed to process chat query after {self.max_retries} attempts: {str(last_error)}")
        raise last_error 