...
async def handle_chat(
    ...
    try:
        logger.info(f"Processing chat request {request_id} for message: '{request_body.message[:50]}...'")
        ...
        # Prepare metadata
        response_metadata = {
            "model_used": result.get("model_used", settings.model_name), # Get model used from result if available
