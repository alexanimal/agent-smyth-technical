...
from ..schemas import HealthStatus, RootResponse
...
async def root(app_state: dict = Depends(get_app_state)):
    ...
    status = (
        ...
        if app_state["is_kb_loading"]
        else "ready" if app_state["chat_handler"] else "error"
    )
    return {"message": "Tweet RAG API is running.", "status": status}
